...
*/
import { Component, OnInit, Input, OnChanges, Output, DoCheck } from '@angular/core';
...
export class CreateSmsTemplateComponent implements OnInit, DoCheck {
  ...
  getSMSType(view?: string) {
    this.schedulerService.getSMSType().subscribe((res: any) => {
      console.log('res', res);
  ...
  getSMSparameter() {
    this.schedulerService.getSMSParameter().subscribe((res: any) => {
      console.log('res', res);
      if (res && res.statusCode === 200) {
  ...
  removeSMSParameterTemplate(parameter: any, index: any) {
    const rem = index - 1;
    this.mappedSMSParameter.data.splice(rem, 1);
  ...
  saveSMStemplate() {
    ...
    this.schedulerService.saveSMSTemplate(requestObject).subscribe((res: any) => {
      console.log('res', res);
      if (res && res.statusCode === 200) {
